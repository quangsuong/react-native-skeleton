...
class StackCardList extends React.Component<StackCardListProps, StackCardListState> {
  scrollXIndex: Animated.Value;
  scrollXAnimated: Animated.Value;

  constructor(props: StackCardListProps) {
    super(props);
  ...
  render() {
    ...
    const spacing = this.props.spacing || 10;
    return (
      <FlatList
        style={{
        ...
        contentContainerStyle={{
          ...
          marginBottom: stackType === 'below' ? spacing * (visibleItems - 1) : 0,
        ...
        CellRendererComponent={(itemProps) => {
          ...
          const newStyle = [
            style,
            {
              zIndex: index < listIndex - 1 ? -100 : this.props?.data?.length - index,
            },
          ];
          return (
