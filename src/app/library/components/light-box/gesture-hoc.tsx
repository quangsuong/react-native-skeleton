...
export const GestureHOC = memo(({ image, source, onClose, backDropOpacity }: GestureHOCProps) => {
  ...
  const top = useDerivedValue(
    () =>
      translateY.value +
      interpolate(animatedProgress.value, [0, 1], [image.py, targetY.value], Extrapolate.CLAMP)
  );

  const left = useDerivedValue(
    () =>
      translateX.value +
      interpolate(animatedProgress.value, [0, 1], [image.px, targetX.value], Extrapolate.CLAMP)
  );
