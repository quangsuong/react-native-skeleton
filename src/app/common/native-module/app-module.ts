...
export const usePhotosPermissionChange = (callback: () => void) => {
  // effect
  useEffect(() => {
    let photosChangeEvent: NativeEventEmitter, subscription: EmitterSubscription;
    if (isIos) {
...
export const setIQKeyboardOption = (options: {
  ...
  if (options.toolbarBarTintColor) {
    actualOption.toolbarBarTintColor = hexStringFromCSSColor(options.toolbarBarTintColor);
  }
  if (options.toolbarTintColor) {
    actualOption.toolbarTintColor = hexStringFromCSSColor(options.toolbarTintColor);
  }
...
export function setToolbarDoneBarButtonItemText(text: string) {
  if (isIos) {
    AppModule.setToolbarDoneBarButtonItemText(text);
  }
}
