...
function useArray<T = any>(initial: T[]): UseArray<T> {
  ...
  const modifyById = useCallback(
    (id, newValue) =>
      setValue((arr) => arr.map((v: any) => (v.id === id ? { ...v, ...newValue } : v))),
    []
  );
...
const useI18nHook = () => {
  const [t] = useTranslation();
  const t18n = useMemo(
    () => (t18n: I18nKeys, t18nOptions?: any) => t18n && t(t18n, t18nOptions),
    []
  );

  const t18nInValid = useMemo(
    () => (t18n: string, t18nOptions?: any) => t18n && t(t18n, t18nOptions),
    []
  );
  return { t18n, t18nInValid };
};
